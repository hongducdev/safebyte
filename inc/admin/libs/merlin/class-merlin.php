...
class Merlin {
	...
	public function admin_page() {
		...
		$texts = array(
			'something_went_wrong' => esc_html__( 'Something went wrong. Please refresh the page and try again!', 'safebyte' ),
		);
	...
	public function svg( $args = array() ) {
		...
		if ( empty( $args ) ) {
			return esc_html__( 'Please define default parameters in the form of an array.', 'safebyte' );
		}

		// Define an icon.
		if ( false === array_key_exists( 'icon', $args ) ) {
			return esc_html__( 'Please define an SVG icon filename.', 'safebyte' );
		}
	...
	public function steps() {
		...
		$this->steps = array(
			'welcome' => array(
				'name'    => esc_html__( 'Welcome', 'safebyte' ),
		...
		if ( class_exists( 'TGM_Plugin_Activation' ) ) {
			$this->steps['plugins'] = array(
				'name' => esc_html__( 'Plugins', 'safebyte' ),
		...
		if ( ! empty( $this->import_files ) ) {
			$this->steps['content'] = array(
				'name' => esc_html__( 'Content', 'safebyte' ),
				'view' => array( $this, 'content' ),
			);
		}

		$this->steps['ready'] = array(
			'name' => esc_html__( 'Ready', 'safebyte' ),
	...
	<?php
		$this->logger->debug( esc_html__( 'The welcome step has been displayed', 'safebyte' ) );
	}
	...
	protected function license() {
		...
		<?php
		$this->logger->debug( esc_html__( 'The license activation step has been displayed', 'safebyte' ) );
	}
	...
	<?php
		$this->logger->debug( esc_html__( 'The child theme installation step has been displayed', 'safebyte' ) );
	}
					...
					<?php foreach ( $required_plugins as $slug => $plugin ) : ?>
						<li data-slug="<?php echo esc_attr( $slug ); ?>">
							...
							<label for="default_plugins_<?php echo esc_attr( $slug ); ?>">
								...
								<span class="badge">
									<span class="hint--top" aria-label="<?php esc_html_e( 'Required', 'safebyte' ); ?>">
										<?php esc_html_e( 'Required', 'safebyte' ); ?>
									</span>
								</span>
							</label>
						</li>
	...
	<?php
		$this->logger->debug( esc_html__( 'The plugin installation step has been displayed', 'safebyte' ) );
	}
			...
			<?php if ( 1 < count( $this->import_files ) ) : ?>
				...
				<div class="merlin__select-control-wrapper">
					...
					<div class="merlin__select-control-help">
						<span class="hint--top" aria-label="<?php echo esc_attr__( 'Select Demo', 'safebyte' ); ?>">
	...
	<?php
		$this->logger->debug( esc_html__( 'The content import step has been displayed', 'safebyte' ) );
	}
	...
	<?php
		$this->logger->debug( esc_html__( 'The final step has been displayed', 'safebyte' ) );
	}
	...
	public function generate_child() {
		...
		if ( ! file_exists( $path ) ) {
			...
			$this->logger->debug( esc_html__( 'The existing child theme was activated', 'safebyte' ) );
		...
		$this->logger->debug( esc_html__( 'The newly generated child theme was activated', 'safebyte' ) );
	...
	public function _ajax_activate_license() {
		...
		if ( ! check_ajax_referer( 'merlin_nonce', 'wpnonce' ) ) {
			wp_send_json(
				array(
					'success' => false,
					'message' => esc_html__( 'Yikes! The theme activation failed. Please try again or contact support.', 'safebyte' ),
				)
			);
		}
		...
		if ( empty( $_POST['license_key'] ) ) {
			wp_send_json(
				array(
					'success' => false,
					'message' => esc_html__( 'Please add your license key before attempting to activate one.', 'safebyte' ),
				)
			);
		}
		...
		$this->logger->debug( esc_html__( 'The license activation was performed with the following results', 'safebyte' ), $result );
	...
	protected function edd_activate_license( $license ) {
		...
		if ( is_wp_error( $response ) || 200 !== wp_remote_retrieve_response_code( $response ) ) {
			...
			if ( is_wp_error( $response ) ) {
				...
				$message = esc_html__( 'An error occurred, please try again.', 'safebyte' );
			}
		} else {
				...
				switch ( $license_data->error ) {
					...
					case 'expired':
						$message = sprintf(
							...
							esc_html__( 'Your license key expired on %s.', 'safebyte' ),
							date_i18n( get_option( 'date_format' ), strtotime( $license_data->expires, current_time( 'timestamp' ) ) )
						);
						break;

					case 'revoked':
						$message = esc_html__( 'Your license key has been disabled.', 'safebyte' );
						break;

					case 'missing':
						$message = esc_html__( 'This appears to be an invalid license key. Please try again or contact support.', 'safebyte' );
						break;

					case 'invalid':
					case 'site_inactive':
						$message = esc_html__( 'Your license is not active for this URL.', 'safebyte' );
						break;

					case 'item_name_mismatch':
						/* translators: EDD Item Name */
						$message = sprintf( esc_html__( 'This appears to be an invalid license key for %s.', 'safebyte' ), $this->edd_item_name );
						break;

					case 'no_activations_left':
						$message = esc_html__( 'Your license key has reached its activation limit.', 'safebyte' );
						break;
	...
	public function generate_child_functions_php( $slug ) {
		...
		$this->logger->debug( esc_html__( 'The child theme functions.php content was generated', 'safebyte' ) );
	...
	public function generate_child_style_css( $slug, $parent, $author, $version ) {
		...
		$this->logger->debug( esc_html__( 'The child theme style.css content was generated', 'safebyte' ) );
	...
	public function generate_child_screenshot( $path ) {
		...
		if ( ! empty( $screenshot ) && file_exists( $screenshot ) ) {
			...
			$this->logger->debug( esc_html__( 'The child theme screenshot was copied to the child theme, with the following result', 'safebyte' ), array( 'copied' => $copied ) );
		} else {
			$this->logger->debug( esc_html__( 'The child theme screenshot was not generated, because of these results', 'safebyte' ), array( 'screenshot' => $screenshot ) );
		}
	}
	...
	function _ajax_plugins() {
		...
		foreach ( $plugins['activate'] as $slug => $plugin ) {
			if ( sanitize_title($_POST['slug']) === $slug ) {
				$json = array(
					...
					'action2'       => - 1,
					'message'       => esc_html__( 'Activating', 'safebyte' ),
				);
				break;
			}
		}
		...
		foreach ( $plugins['update'] as $slug => $plugin ) {
			if ( sanitize_title($_POST['slug']) === $slug ) {
				$json = array(
					...
					'action2'       => - 1,
					'message'       => esc_html__( 'Updating', 'safebyte' ),
				);
				break;
			}
		}
		...
		foreach ( $plugins['install'] as $slug => $plugin ) {
			if ( sanitize_title($_POST['slug']) === $slug ) {
				$json = array(
					...
					'action2'       => - 1,
					'message'       => esc_html__( 'Installing', 'safebyte' ),
				);
				break;
			}
		}
		...
		if ( $json ) {
			$this->logger->debug(
				__( 'A plugin with the following data will be processed', 'safebyte' ),
				array(
			...
			$json['message'] = esc_html__( 'Installing', 'safebyte' );
			wp_send_json( $json );
		} else {
			$this->logger->debug(
				__( 'A plugin with the following data was processed', 'safebyte' ),
				array(
					'plugin_slug' => sanitize_title($_POST['slug']),
				)
			);
			...
			wp_send_json(
				array(
					'done'    => 1,
					'message' => esc_html__( 'Success', 'safebyte' ),
				)
			);
		}
	...
	function _ajax_content() {
		...
		if ( ! check_ajax_referer( 'merlin_nonce', 'wpnonce' ) || empty( $_POST['content'] ) && isset( $content[ $_POST['content'] ] ) ) {
			$this->logger->error( esc_html__( 'The content importer AJAX call failed to start, because of incorrect data', 'safebyte' ) );

			wp_send_json_error(
				array(
					'error'   => 1,
					'message' => esc_html__( 'Invalid content!', 'safebyte' ),
				)
			);
		}
		...
		if ( isset( $_POST['proceed'] ) ) {
			if ( is_callable( $this_content['install_callback'] ) ) {
				$this->logger->info(
					__( 'The content import AJAX call will be executed with this import data', 'safebyte' ),
					array(
		...
		if ( $json ) {
			...
			$this->logger->error(
				__( 'The content import AJAX call failed with this passed data', 'safebyte' ),
				array(
			...
			wp_send_json(
				array(
					'error'   => 1,
					'message' => esc_html__( 'Error', 'safebyte' ),
					'logs'    => '',
					'errors'  => '',
				)
			);
	...
	public function _ajax_get_total_content_import_items() {
		if ( ! check_ajax_referer( 'merlin_nonce', 'wpnonce' ) && empty( $_POST['selected_index'] ) ) {
			$this->logger->error( esc_html__( 'The content importer AJAX call for retrieving total content import items failed to start, because of incorrect data.', 'safebyte' ) );

			wp_send_json_error(
				array(
					'error'   => 1,
					'message' => esc_html__( 'Invalid data!', 'safebyte' ),
				)
			);
		}
	...
	protected function get_import_data( $selected_import_index = 0 ) {
		...
		if ( ! empty( $import_files['content'] ) ) {
			$content['content'] = array(
				...
				'description'      => esc_html__( 'Demo content data.', 'safebyte' ),
				'pending'          => esc_html__( 'Pending', 'safebyte' ),
				'installing'       => esc_html__( 'Installing', 'safebyte' ),
				'success'          => esc_html__( 'Success', 'safebyte' ),
		...
		if ( ! empty( $import_files['widgets'] ) ) {
			$content['widgets'] = array(
				'title'            => esc_html__( 'Widgets', 'safebyte' ),
				'description'      => esc_html__( 'Sample widgets data.', 'safebyte' ),
				'pending'          => esc_html__( 'Pending', 'safebyte' ),
				'installing'       => esc_html__( 'Installing', 'safebyte' ),
				'success'          => esc_html__( 'Success', 'safebyte' ),
		...
		if ( ! empty( $import_files['sliders'] ) ) {
			$content['sliders'] = array(
				'title'            => esc_html__( 'Revolution Slider', 'safebyte' ),
				'description'      => esc_html__( 'Sample Revolution sliders data.', 'safebyte' ),
				'pending'          => esc_html__( 'Pending', 'safebyte' ),
				'installing'       => esc_html__( 'Installing', 'safebyte' ),
				'success'          => esc_html__( 'Success', 'safebyte' ),
		...
		if ( ! empty( $import_files['options'] ) ) {
			$content['options'] = array(
				'title'            => esc_html__( 'Options', 'safebyte' ),
				'description'      => esc_html__( 'Sample theme options data.', 'safebyte' ),
				'pending'          => esc_html__( 'Pending', 'safebyte' ),
				'installing'       => esc_html__( 'Installing', 'safebyte' ),
				'success'          => esc_html__( 'Success', 'safebyte' ),
		...
		if ( ! empty( $import_files['redux'] ) ) {
			$content['redux'] = array(
				'title'            => esc_html__( 'Redux Options', 'safebyte' ),
				'description'      => esc_html__( 'Redux framework options.', 'safebyte' ),
				'pending'          => esc_html__( 'Pending', 'safebyte' ),
				'installing'       => esc_html__( 'Installing', 'safebyte' ),
				'success'          => esc_html__( 'Success', 'safebyte' ),
		...
		if ( false !== has_action( 'merlin_after_all_import' ) ) {
			$content['after_import'] = array(
				'title'            => esc_html__( 'After import setup', 'safebyte' ),
				'description'      => esc_html__( 'After import setup.', 'safebyte' ),
				'pending'          => esc_html__( 'Pending', 'safebyte' ),
				'installing'       => esc_html__( 'Installing', 'safebyte' ),
				'success'          => esc_html__( 'Success', 'safebyte' ),
	...
	public function import_revolution_sliders( $file ) {
		...
		$this->logger->info( esc_html__( 'The revolution slider import was executed', 'safebyte' ) );
	...
	public function pt_importer_new_ajax_request_response_data( $data ) {
		...
		$data['message']  = esc_html__( 'Installing', 'safebyte' );
		$data['proceed']  = 'true';
	...
	public function after_content_import_setup() {
		...
		if ( $homepage ) {
			...
			$this->logger->debug( esc_html__( 'The home page was set', 'safebyte' ), array( 'homepage_id' => $homepage ) );
		}
		...
		if ( $blogpage ) {
			...
			$this->logger->debug( esc_html__( 'The blog page was set', 'safebyte' ), array( 'blog_page_id' => $blogpage ) );
		}
	}
	...
	public function before_content_import_setup() {
		...
		if ( ! empty( $hello_world ) ) {
			...
			$this->logger->debug( esc_html__( 'The Hello world post status was set to draft', 'safebyte' ) );
		}
	}
	...
	public function validate_import_file_info( $import_files ) {
		...
		foreach ( $import_files as $import_file ) {
			if ( ! empty( $import_file['import_file_name'] ) ) {
				...
				$this->logger->warning( esc_html__( 'This predefined demo import does not have the name parameter: import_file_name', 'safebyte' ), $import_file );
			}
		}
