...
if (!class_exists('Safebyte_Page')) {

    class Safebyte_Page
    {
        public function get_site_loader(){

            $site_loader = safebyte()->get_theme_opt( 'site_loader', 'off' );
            $site_loader_style = safebyte()->get_theme_opt( 'site_loader_style', 'style-1' );
            $site_loader_icon = safebyte()->get_theme_opt( 'site_loader_icon' );
            if($site_loader == 'on') { ?>
        ...
        public function get_page_title(){
            ...
            $pt_mode = safebyte()->get_opt('pt_mode');
            $pt_mode_product = safebyte()->get_opt('pt_mode_product');
            $ptitle_scroll_opacity = safebyte()->get_opt('ptitle_scroll_opacity');
            $ptitle_layout = (int)safebyte()->get_opt('ptitle_layout');
            $ptitle_layout_product = (int)safebyte()->get_opt('ptitle_layout_product');
            ...
            <?php } elseif($pt_mode == 'df') {
                $ptitle_breadcrumb_on = safebyte()->get_opt( 'ptitle_breadcrumb_on', '1' ); 
        ...
        public function get_title() {
            ...
            if ( ! is_archive() ) {
                ...
                elseif ( is_page() ) {
                    ...
                    $title = esc_html__( '404 Error', 'safebyte' );
                } elseif ( is_search() ) {
                    $title = esc_html__( 'Search results', 'safebyte' );
                } elseif ( is_singular('lp_course') ) {
                    $title = esc_html__( 'Course', 'safebyte' );
                } else {
                    $title = get_post_meta( get_the_ID(), 'custom_title', true );
                    if ( ! $title ) {
                        $title = get_the_title();
        ...
        public function get_pagination( $query = null, $ajax = false ){
            ...
            if($ajax){
                add_filter('paginate_links', 'safebyte_ajax_paginate_links');
            }
