...
class Safebyte_CSS_Generator {
	...
	function __construct() {
		$this->opt_name = safebyte()->get_option_name();  
        ...
        add_action( 'wp_enqueue_scripts', array( $this, 'safebyte_enqueue' ), 20 );
	}
	...
	function init() {
		...
		add_action( 'wp', array( $this, 'safebyte_generate_with_dev_mode' ) );
		add_action( "redux/options/{$this->opt_name}/saved", function () {
			$this->safebyte_generate_file_options();
		} );
	}

	function safebyte_generate_with_dev_mode() {
		if ( $this->dev_mode === true ) {
            $this->safebyte_generate_file_options();
			$this->safebyte_generate_file();
		}
	}

    function safebyte_generate_file_options() {
        ...
        $this->redux->filesystem->execute( 'put_contents', $_options, array(
            'content' => preg_replace( "/(?<=[^\r]|^)\n/", "\r\n", $this->safebyte_options_output() )
        ) );
    }

	function safebyte_generate_file() {
		$scss_dir = get_template_directory() . '/assets/scss/';
	...
	protected function safebyte_options_output() {
		$theme_colors                    = safebyte_configs('theme_colors');
        $links                           = safebyte_configs('link');
        $gradients                       = safebyte_configs('gradient');
        $gradients2                      = safebyte_configs('gradient2');
		ob_start();

		printf('$safebyte_theme_colors_key:(%s);',$this->print_scss_opt_colors($theme_colors,'key'));
        printf('$safebyte_theme_colors_val:(%s);',$this->print_scss_opt_colors($theme_colors,'val'));
        // color rgb only
        ...
        /* Font */
        $font_secondary = safebyte()->get_theme_opt('f_secondary');
    ...
    /* Inline CSS */
    function safebyte_enqueue() {
        $css = $this->safebyte_inline_css();
        if ( !empty( $css ) ) {
            wp_add_inline_style( 'pxl-style', $css );
        }
    }
    protected function safebyte_inline_css() {
        ob_start();
